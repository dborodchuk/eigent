...
## Resume blocked during a plain terminal wait

### Cause and fix

An interrupted `shell_exec` call was recorded as `unsafe_write` with an unknown
outcome even when the command was just `sleep 60`. The generic shell path can
have external effects, so the journal correctly refused to replay it. The
controller returned only `unsafe_resume_blocked`, which surfaced unchanged in
the error toast.

The built-in TerminalToolkit now executes a plain foreground `sleep N` as a
bounded in-process delay, without launching a shell, loading profiles, resolving
PATH commands, or acquiring a workspace writer. Only literal nonnegative
numeric durations up to 600 seconds, covered by the requested timeout, qualify.
The concrete built-in FunctionTool carries an argument-guarded `safe_read`
declaration using the same predicate as execution. Other shell commands,
background work, and untrusted tools with the same name remain conservative.
Guard failures and exceptions also remain conservative.

The conflict response retains its code and blocker IDs and now supplies an
explanation that an earlier action has an unknown outcome. It suggests sending
a new task to inspect existing work and continue what remains.

### Validation and limits

- 100 backend tests passed across TerminalToolkit, tool checkpoints, run
  control, and journal recovery. Coverage includes cancellation and restart
  during a plain wait, compound shell commands that must still block Resume,
  argument guard rejection, and execution without shell/workspace side effects.
  Tests used an isolated temporary `file_save_path`.
- Six HTTP client tests passed, including preference for the explanation while
  retaining the 409 status, error code, and blocker IDs. Type checking, focused
  ESLint, Ruff, and `git diff --check` passed.
- Electron rendered the actual HTTP client's error through the shared Toaster
  in light and dark themes using a local 409 fixture and isolated auth state.
  The complete explanation was visible in both themes. Reused Toaster and its
  existing neutral/Ink/hairline/elevation tokens; no layout, tokens, or design
  exceptions changed. Narrow windows and localized server errors were not
  checked; this is an English backend error message.
- Existing unknown-outcome records are not rewritten: their commands ran
  through the previous generic shell implementation. A separately submitted
  task can inspect the saved files and continue. New executions require the
  updated backend process. An interrupted in-process delay may be retried;
  it does not establish that the requested waiting period finished.
- Same-task Resume admission is tested against the real journal, including
  simulated restart. A new live model-driven quit/resume cycle was not run.
